	...
	1: {
		...
		run: function() {
			try {
				...
				console.log(chalk.bold.green("Failed!"));
			}
		}
	},
	2: {
...
if (fs.existsSync(__dirname + "/.autorun")) {
	...
	console.clear();
	console.log(chalk.gold(figlet.textSync('Mirai Panel', { horizontalLayout: 'full' })));
	console.log(chalk.green("	================== © 2020 CatalizCS ==================\n"));
	console.log(
	...
	console.log(chalk.gold("	============ " + chalk.bold.magenta("Hãy chọn một trong những tuỳ chọn bên dưới") + " ============\n"));
	let choose = "";
	...
	rl.on('line', async (input) => {
		...
		if (!input) {
			console.log(chalk.bold.green("Bạn cần phải lựa chọn và nhập các tuỳ chọn có sẵn ở trên"));
			rl.prompt();
			return;
		}
		if (input in commands) {
			...
			console.log(chalk.bold.gold("Tuỳ chọn của bạn không tồn tại"));
			rl.prompt();
			return;
		}
	});
