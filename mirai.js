...
try {
	...
	logger("Config Loaded!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
}
catch (error) {
	return logger("Không thể load config!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
}
...
axios.get('https://raw.githubusercontent.com/catalizcs/miraiv2/master/package.json').then((res) => {
	logger("Đang kiểm tra cập nhật...", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	var local = JSON.parse(readFileSync('./package.json')).version;
	if (semver.lt(local, res.data.version)) logger(`Đã có phiên bản ${res.data.version} để bạn có thể cập nhật!`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG CHECK UPDATE ]");
	else logger('Bạn đang sử dụng bản mới nhất!', "[ >>❕<< ĐÃ LOADER THÀNH CÔNG CHECK UPDATE ]");
}).catch(err => logger("Đã có lỗi xảy ra khi đang kiểm tra cập nhật cho bạn!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG CHECK UPDATE ]"));
	...
	try {
		...
		if (command.config.dependencies) {
			try {
				...
				logger(`Không tìm thấy gói phụ trợ cho module ${command.config.name}, tiến hành cài đặt: ${command.config.dependencies.join(", ")}!`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
				execSync('npm install -s ' + command.config.dependencies.join(" "));
				delete require.cache[require.resolve(`./modules/commands/${file}`)];
				logger(`Đã cài đặt thành công toàn bộ gói phụ trợ cho module ${command.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
			}
		}
        if (command.config.envConfig) {
            try {
                ...
                logger(`Loaded config module ${command.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]")
            } catch (error) {
                console.log(error);
                logger(`Không thể tải config module ${command.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
            }
        }
		client.commands.set(command.config.name, command);
		logger(`Loaded command ${command.config.name}!`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	}
	catch (error) {
		logger(`Không thể load module command ${file} với lỗi: ${error.message}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	}
}
	...
	try {
		...
		if (event.config.dependencies) {
			try {
				...
				logger(`Đã cài đặt thành công toàn bộ gói phụ trợ cho event module ${event.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
			}
		}
        if (event.config.envConfig) {
            try {
                ...
                logger(`Loaded config event module ${event.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]")
            } catch (error) {
                logger(`Không thể tải config event module ${event.config.name}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
            }
        }
		client.events.set(event.config.name, event);
		logger(`Loaded event ${event.config.name}!`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	}
	catch (error) {
		logger(`Không thể load module event ${file} với lỗi: ${error.message}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	}
}

logger(`Load thành công: ${client.commands.size} module commands | ${client.events.size} module events`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]")
writeFileSync(client.globalConfig, JSON.stringify(config, null, 4));

execSync('npm cache clean --force');
logger("Npm cache cleaned!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG ]");
	...
	login({ appState: require(appStateFile) }, (err, api) => {
		...
		try {
			...
			setInterval(() => {
				...
				setTimeout(() => onListen(), 1800);
				if (__GLOBAL.settings.DeveloperMode == true) {
					const moment = require("moment");
					var time = moment.tz("Asia/Ho_Chi_minh").format("HH:MM:ss L");
					logger(`[ ${time} ] Listen restarted`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG DEV MODE ]");
				}
			}, 1800000);
		}
		catch(e) {
			logger(`${e.name}: ${e.message}`, "[ >>❕<< ĐÃ LOADER THÀNH CÔNG LISTEN ]")
		}
	});
}
...
sequelize.authenticate().then(
	() => logger("Kết nối cơ sở dữ liệu thành công!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG DATABASE ]"),
	() => logger("Kết nối cơ sở dữ liệu thất bại!", "[ >>❕<< ĐÃ LOADER THÀNH CÔNG DATABASE ]")
).then(() => {
